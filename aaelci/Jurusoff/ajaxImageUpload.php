		   ...
		   if (strpos($file2,'_I-01')||strpos($file2,'_I_01')||strpos($file2,'_I01')) {$jdok='01';$pr='1';}
			...
			elseif (strpos($file2,'_PDL')||strpos($file2,'_DIL')||strpos($file2,'_pdl')||strpos($file2,'dil')) {$jdok='12';$npdl='1';}
			   ...
			   $Qry="select ltrim(to_char(max(no_img)+1,'000')) no_baru from cust_ail_img 
				...
				if ($datax[0]>=1){
				echo "<script type='text/javascript'>confirm('File sudah PERNAH di UPLOAD..-> $filename!');</script>";
				exit;
				}
...
//echo "<script type='text/javascript'>alert('5a');</script>";			
			oci_error($sql);  // For oci_execute errors pass the statement handle
...
//	 echo "<script type='text/javascript'>alert('siap close');</script>";	
	 oci_close($cn);
